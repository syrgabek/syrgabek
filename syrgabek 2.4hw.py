...
class GameEntity:
    def __init__(self, name,name_attack, health, damage, rndom):
        self.__name = name
...
class Berserk(Hero):
    def __init__(self, name, health, damage,critical_Strike ):
        super().__init__(name, health, damage, 'Critical_Strike')
        self.__critical_Strike = critical_Strike

    def apply_super_ability(self, boss, heroes):
        if self.health > 0:
            if self.health - boss.health:
                self.ritical_Strike = self.damage + int(boss.damage /2)
                self.damage = self.__critical_Strike

class Thor(Hero):
    def __init__(self, name, health, damage, grom ):
        super().__init__(name, health, damage, 'grom')
        self.__grom = self.__grom

    def apply_super_ability(self, boss, heroes):
        if self.health > 0 and round_number % 2 != 0:
            self.damage = self.__grom
        if self.damage == self.__grom:
            boss.damage = 50
        else:
            boss.damage = 0

class Golem(Hero):
    def __init__(self, name, health, damage, fireplace_man):
        super().__init__(name, health, damage,'fireplace_man')
        self.__fireplace_man = fireplace_man
...
def start():
    ...
    thor = Thor("Thor", random.randint(600, 800), 5)
    heroes = [warrior, medic_1, medic_2, magic,thor,berserk ,]
